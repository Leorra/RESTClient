...
// Undefine platform macros that collide with our enum values.
#ifdef ERROR
#undef ERROR
#endif
#ifdef INFO
...
namespace restclient {
	...
	struct RESTConfig {
		...
		std::string          auth_host { "api.coinbase.com" };  // kept lowercase -- JWT URI uses this verbatim
	...
	public:
		...
		~RESTClient() {
			// Signal the worker thread to stop after draining remaining tasks.
			// We take async_mtx_ so the worker's predicate sees the updated flag
			// atomically with any in-flight enqueue.
			{
		...
		[[nodiscard]] Response send(std::string_view endpoint,
			...
			std::string_view body = "")
		{
			// --- Rate throttling (does not hold transport lock) ---
			{
			...
			const std::string method_str(method);   // null-terminated copy for libcurl

			// Build a per-request auth node and temporarily append it to the
			// static header list.  The static list tail's ->next is always nullptr
			// outside this critical section (guarded by transport_mtx_).
			...
			if (static_headers_tail_) {
				static_headers_tail_->next = auth_node;   // extend list
			}
			...
			// Always restore the static list tail before releasing auth_node.
		...
		//        On Win32 UI threads use PostMessage/PostThreadMessage to marshal
		//        the result back to the message loop.
		// ---------------------------------------------------------
	...
	private:
		...
		void enqueue_task(std::string  ep,
			...
			AsyncCallback cb,
			bool          with_retry) {
			// `cb` is consumed (moved) only if the task is accepted.
			// If the queue is full, cb is still valid and we invoke it inline
			// with a rejection error -- no copies are needed.
		...
		void log(RESTLogLevel level, std::string_view message) const {
			...
			std::cout << '[' << std::put_time(&buf, "%Y-%m-%d %H:%M:%S") << "] "
				<< message << '\n';   // '\n' avoids the flush overhead of std::endl
		}
		...
		[[nodiscard]] std::string generate_nonce() const {
			...
			// EVP_ENCODE_LENGTH(n) adds one extra byte for a newline; we use the
			// exact formula instead to avoid a trailing '\n' in the nonce string.
		...
		static constexpr RESTErrorType classify_curl_error(CURLcode code) noexcept {
			switch (code) {
				...
				case CURLE_SSL_ENGINE_SETFAILED: [[fallthrough]];
					// CURLE_PEER_FAILED_VERIFICATION == 60; same numeric value as the
					// deprecated CURLE_SSL_CACERT -- use only the canonical name.
